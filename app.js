...
if (fs.existsSync("./config.json")) {
    ...
    if (compareKeys(config, require("./config.example.json"))) {
        log(
            "Config file has been changed, please check config.example.json"
        );
    }
} else {
    log(
...
if (config.username === "" || config.password === "") {
    log(
        "Edit config.json! Add you username and password and start the bot"
    );
    process.exit(1);
}
...
client.on('loggedOn', function (details, parental) {
    ...
    client.getPersonas([client.steamID], function (err, steamid) {
        ...
        log('Idling: ' + games.length + ' games, getting ' + (games.length * 24) + ' hours per day | ' + (games.length * 336) + ' hours per 2 weeks');
...
client.on('friendMessage', function (steamid, message) {
    if (config.sendautomessage && responded.indexOf(steamid.getSteamID64()) === -1) {
...
client.on('lobbyInvite', function (inviterID, lobbyID) {
    if (config.sendautomessage && responded.indexOf(inviterID.getSteamID64()) === -1) {
